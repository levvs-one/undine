...
| `shaders/` | GLSL ES 3.00 for the spectral surface step (FFT pass, mode evolution, forces), the caustics and the pool render, for the shallow-water flow step and the table render, and for the particle liquid (the whole position-based scheme, and its screen-space rendering) (what the site runs); an HLSL file for Unity and Unreal; a Godot 4 shader. |
