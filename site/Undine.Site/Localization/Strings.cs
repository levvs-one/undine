...
public static class Strings
{
    ...
    public static readonly Dictionary<string, (string En, string Ru)> Table = new(StringComparer.Ordinal)
    {
        ...
        ["home.lede"] = ("A pool on the GPU. The surface is a wave equation run at the liquid's own ripple speed and damped by its own viscosity; the light through it is Caustikon's optics for that liquid; the lamp's caustics on the floor and walls come from refracted rays, not from a texture. Touch the surface.", "Бассейн на GPU. Поверхность — волновое уравнение со скоростью ряби этой жидкости и с её же вязкостью; свет сквозь неё — оптика Caustikon для этой жидкости; каустики лампы на дне и стенах — из преломлённых лучей, а не из текстуры. Тронь поверхность."),
        ...
        ["home.r.speed"] = ("ripple speed the field runs at", "скорость ряби, с которой идёт поле"),
        ...
        ["home.what.1"] = ("<strong>The surface</strong> is a height field: each cell keeps a height and a vertical velocity, and each substep applies v += c²∇²h·dt + ν∇²v·dt − γv·dt, h += v·dt. The field runs at one speed c, the liquid's phase speed for the wavelength the touches make, from the full gravity–capillary relation c² = (g/k + σk/ρ)·tanh(kh); a 2 cm ripple on water runs at a quarter of a metre a second, not at the √(g·h) of long waves. ν is the kinematic viscosity, which is why ripples on glycerol die at once and ripples on water would ring for minutes; γ is what a pool's rim and surface film take out beyond that, and it is a slider; so is the wind, a random pressure on the surface that keeps a pool from ever lying flat. The substep is chosen from the stability bounds; the readout shows it.", "<strong>Поверхность</strong> — поле высот: каждая ячейка хранит высоту и вертикальную скорость, каждый подшаг применяет v += c²∇²h·dt + ν∇²v·dt − γv·dt, h += v·dt. Поле идёт с одной скоростью c — фазовой скоростью жидкости для длины волны, которую делают касания, из полного гравитационно-капиллярного соотношения c² = (g/k + σk/ρ)·tanh(kh); рябь 2 см по воде идёт четверть метра в секунду, а не √(g·h) длинных волн. ν — кинематическая вязкость, поэтому рябь на глицерине гаснет сразу, а на воде звенела бы минутами; γ — то, что сверх этого забирают бортик и плёнка на поверхности, и это ползунок; ветер тоже — случайное давление на поверхность, из-за которого бассейн никогда не лежит ровно. Подшаг выбирается из условий устойчивости; readout его показывает."),
        ...
        ["about.model.p"] = ("The surface is a height field: v += c²∇²h·dt + ν∇²v·dt − γv·dt, h += v·dt, with ν = η/ρ and c the liquid's phase speed for the wavelength the touches make, from c² = (g/k + σk/ρ)·tanh(kh); γ is the decay a pool's rim and surface film add, and the wind is a random pressure on the surface. The substep is four fifths of the smaller stability bound. Light: exact Fresnel per channel, Snell's law per channel, Beer–Lambert absorption over the path, caustics as the area ratio of the refracted surface on the floor and, by continuing the refracted beam, on the walls. Not modelled yet: dispersion inside one scene (the field runs at one speed; the package gives the true speed for every wavelength), breaking, splashes, foam, floating bodies, light reflected off the floor back up through the surface.", "Поверхность — поле высот: v += c²∇²h·dt + ν∇²v·dt − γv·dt, h += v·dt, где ν = η/ρ, а c — фазовая скорость жидкости для длины волны, которую делают касания, из c² = (g/k + σk/ρ)·tanh(kh); γ — затухание, которое добавляют бортик и плёнка на поверхности, ветер — случайное давление на поверхность. Подшаг — четыре пятых меньшего из условий устойчивости. Свет: точный Френель по каналам, закон Снелла по каналам, поглощение Бугера–Ламберта вдоль пути, каустики как отношение площадей преломлённой поверхности на дне и, продолжением преломлённого пучка, на стенах. Не смоделировано пока: дисперсия внутри одной сцены (поле идёт с одной скоростью; пакет даёт истинную скорость для любой длины волны), обрушение, брызги, пена, плавающие тела, свет, отражённый от дна обратно вверх сквозь поверхность."),
