...
public static class Strings
{
    ...
    public static readonly Dictionary<string, (string En, string Ru)> Table = new(StringComparer.Ordinal)
    {
        ...
        ["home.what.3"] = ("<strong>The caustics</strong> are rays: one per grid point and colour channel, refracted by the lamp's direction and the local normal, landing on the floor plane and summed into a map that reaches past the floor so the walls can read it too. A flat surface lands every ray evenly, so the map reads one; a ripple focuses rays into a bright line and leaves a darker band beside it. The map is rebuilt every frame.", "<strong>Каустики</strong> — лучи: по одному на точку сетки и цветовой канал, преломлённые направлением лампы и локальной нормалью, падающие на плоскость дна и суммируемые в карту, которая выходит за дно, чтобы её читали и стены. Ровная поверхность кладёт лучи равномерно, и карта читается как единица; рябь собирает лучи в яркую линию и оставляет рядом тёмную полосу. Карта строится заново каждый кадр."),
        ...
        ["about.model.p"] = ("The surface is a height field: v += c²∇²h·dt + ν∇²v·dt − γv·dt, h += v·dt, with ν = η/ρ and c the liquid's phase speed for the wavelength the touches make, from c² = (g/k + σk/ρ)·tanh(kh); γ is the decay a pool's rim and surface film add, and the wind is a random pressure on the surface. The substep is bounded by both the wave and the diffusion stability limits. Light: exact Fresnel per channel, Snell's law per channel, Beer–Lambert absorption over the path, caustics by ray counting on the floor and, by continuing the refracted beam, on the walls. Not modelled yet: dispersion inside one scene (the field runs at one speed; the package gives the true speed for every wavelength), breaking, splashes, foam, floating bodies, light reflected off the floor back up through the surface.", "Поверхность — поле высот: v += c²∇²h·dt + ν∇²v·dt − γv·dt, h += v·dt, где ν = η/ρ, а c — фазовая скорость жидкости для длины волны, которую делают касания, из c² = (g/k + σk/ρ)·tanh(kh); γ — затухание, которое добавляют бортик и плёнка на поверхности, ветер — случайное давление на поверхность. Подшаг ограничен и волновым, и диффузионным условием устойчивости. Свет: точный Френель по каналам, закон Снелла по каналам, поглощение Бугера–Ламберта вдоль пути, каустики подсчётом лучей на дне и, продолжением преломлённого пучка, на стенах. Не смоделировано пока: дисперсия внутри одной сцены (поле идёт с одной скоростью; пакет даёт истинную скорость для любой длины волны), обрушение, брызги, пена, плавающие тела, свет, отражённый от дна обратно вверх сквозь поверхность."),
